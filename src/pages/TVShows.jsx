...
export default function TVShows() {
  ...
  useEffect(() => {
    tmdb.genreTV().then(d => setGenres(d.genres || [])).catch(() => {})
  }, [])

  const resetKey = `${sortKey}-${activeGenre}`
  const fetchFn = (p) => tmdb.discoverTV(p, activeGenre, `${sortKey}.desc`)
    ...
    <div className="pt-20 max-w-7xl mx-auto px-3 sm:px-4 pb-12 space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="font-display text-2xl uppercase tracking-wider"
          style={{ color: '#c8e8ff', textShadow: '0 0 20px rgba(106,26,154,0.5)' }}>
          TV Shows
        </h1>

        <div className="flex gap-1.5 rounded p-1"
          style={{ background: 'rgba(13,22,38,0.9)', border: '1px solid rgba(106,26,154,0.2)' }}>
          {SORTS.map(s => (
      ...
      {genres.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
          <GenreChip label="All" active={!activeGenre} onClick={() => setActiveGenre(null)} tv />
          {genres.map(g => (
            <GenreChip key={g.id} label={g.name} active={activeGenre === g.id} onClick={() => setActiveGenre(g.id === activeGenre ? null : g.id)} tv />
          ))}
        </div>
      )}
...
function GenreChip({ label, active, onClick, tv }) {
  return (
    <button
      onClick={onClick}
      className="shrink-0 px-3 py-1 rounded-full text-xs font-display uppercase tracking-wider transition-all"
      style={active ? {
        background: tv ? 'linear-gradient(135deg, #6a1a9a, #4a0a7a)' : 'linear-gradient(135deg, #1a7fd4, #0d4a8a)',
        color: '#fff',
        border: tv ? '1px solid rgba(106,26,154,0.5)' : '1px solid rgba(100,180,255,0.4)',
        boxShadow: tv ? '0 0 10px rgba(106,26,154,0.5)' : '0 0 10px rgba(26,127,212,0.5)',
      } : {
        background: 'rgba(13,22,38,0.7)',
        color: '#2a5a8a',
        border: tv ? '1px solid rgba(106,26,154,0.15)' : '1px solid rgba(26,127,212,0.15)',
      }}
    >
      {label}
    </button>
