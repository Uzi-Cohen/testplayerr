  ...
  return (
    <Link to={`/watch/${mediaType}/${item.id}`} className="group block card-2010">
      <div className="aspect-[2/3] relative overflow-hidden" style={{ background: '#0a1220' }}>
        ...
        {/* Synopsis overlay — slides up on hover */}
        <div
          className="absolute inset-0 flex flex-col justify-end transition-all duration-300"
          style={{
            background: 'linear-gradient(0deg, rgba(5,10,20,0.98) 0%, rgba(5,10,20,0.7) 50%, rgba(5,10,20,0.1) 100%)',
            opacity: 0,
          ...
          ref={el => {
            ...
            const hide = () => { el.style.opacity = '0'; el.style.transform = 'translateY(6px)' }
          ...
          <div className="p-2.5 space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0"
                style={{
                  ...
                  boxShadow: '0 0 14px rgba(26,127,212,0.8)',
            ...
            {item.vote_average > 0 && (
              <div className="flex items-center gap-1">
                {[1,2,3,4,5].map(s => (
                  <svg key={s} className="w-2.5 h-2.5" fill={s <= Math.round(item.vote_average / 2) ? '#ffc107' : '#1a3050'} viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
                <span className="text-xs ml-0.5" style={{ color: '#4a6a8a', fontSize: '9px' }}>{item.vote_average.toFixed(1)}</span>
              </div>
            )}

            {/* Overview */}
            {overview && (
              <p className="text-xs leading-snug line-clamp-3" style={{ color: '#4a6a8a', fontSize: '10px' }}>
                {overview}
              </p>
            )}
          </div>
      ...
      <div className="px-2 py-2" style={{ background: 'linear-gradient(180deg, #0d1626, #080f1c)' }}>
        ...
        <div className="flex items-center justify-between mt-0.5">
          <span style={{ color: '#3a5a7a', fontSize: '10px' }}>{formatDate(date)}</span>
          {item.vote_average > 0 && (
            <span style={{ color: '#ffc107', fontSize: '10px' }}>★ {item.vote_average.toFixed(1)}</span>
          )}
        </div>
      </div>
    </Link>
